(function(root, factory) {
    ...
    function Compile(structure, opts) {
        ...
        // Cache for deduplicating multi-char string literal helper rules.
        // Both objects are shared across @include boundaries via the opts.__proto__ chain,
        // mirroring the existing alreadycompiled mechanism.
        if (!opts.stringLiteralCache) {
            opts.stringLiteralCache = Object.create(null);
        }
        if (!opts.stringLiteralCounter) {
            opts.stringLiteralCounter = {n: 0};
        }
        ...
        function buildStringToken(ruleName, token, env) {
            // Return the existing rule if we already built this string literal.
            var cached = opts.stringLiteralCache[token.literal];
            if (cached) return cached;
            // Use a content-neutral name ($string$N) rather than a ruleName-based
            // name so that error messages don't show a misleading rule ownership
            // (e.g. "a$string$1" appearing in b's error trace).
            // The "$" prefix is reserved for generated names and cannot be written
            // in .ne source, so there is no collision risk.
            var newname = "$string$" + (++opts.stringLiteralCounter.n);
            opts.stringLiteralCache[token.literal] = newname;
